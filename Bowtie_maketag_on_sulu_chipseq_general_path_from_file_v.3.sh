...
echo "You are now in: $_mydir"
 ...
 case "${seqtype}" in
       single)
            ...
            gunzip -c ${SAMPLE}/*.fastq.gz > ${SAMPLE}/${j}.bowtie/${j}.fastq
            Trimmode=SE
            Trimadapter=/rothenberg/sulu/data00/Trimmomatic/adapters/TruSeq3-SE.fa
       ;;
            #OPTION: paired-end data data
       paired)
            gunzip -c ${SAMPLE}/*R1*.fastq.gz > ${SAMPLE}/${j}.bowtie/${j}.R1.fastq
            gunzip -c ${SAMPLE}/*R2*.fastq.gz > ${SAMPLE}/${j}.bowtie/${j}.R2.fastq
            Trimmode=PE
       ...
       ;;
        ...
        if [ ${trimstatus} == yes ]; then
            ...
            java -jar /rothenberg/sulu/data00/Trimmomatic/trimmomatic-0.33.jar ${Trimmode} -threads ${threads} -trimlog ${j}.trimlog.txt ${TRIMfastqin} ${TRIMfastqout} ILLUMINACLIP:${Trimadapter}:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:25 2>> $RUNLOG
            ...
            if [ ${seqtype} = single ]; then
                ...
                echo "What can of data type did you choose up there. Something is upt with the script!!!"
            fi
        fi
  ...
  /rothenberg/sulu/data00/bowtie/bowtie /rothenberg/sulu/data00/bowtie-indexes/${refgenome} -p ${threads} ${bowtiesettings} -q ${bowtiefastq} --un ${unmapped_out} --max ${j}.${refgenome}.repeat.fq -S ${j}.${refgenome}.sam 2>> $RUNLOG
...
#######################Remove duplicates and masked regions
        ...
        if [ ${rmdup} == yes ] || [ ${rmsk} == yes ]; then
            ...
            "yes|yes")
                ...
                #SamToBam
                echo "Creating and indexing ${j}.s.bam"
                samtools view -bS ${j}.${refgenome}.sam > ${j}.bam 2>> $RUNLOG
                samtools sort ${j}.bam ${j}.s 2>> $RUNLOG
                rm ${j}.bam
                samtools index ${j}.s.bam 2>> $RUNLOG
                #remove duplicate reads from the sorted bam-file
                echo "Removing duplicate reads from ${j}.s.bam"
                samtools rmdup ${rmdup_seqtype} ${j}.s.bam ${j}.rmdup.bam 2>> $RUNLOG
                #Create the bed-file
                echo "bamToBed -i ${j}.rmdup.bam > ${j}.rmdup.bed"
                bamToBed -i ${j}.rmdup.bam > ${j}.rmdup.bed 2>> $RUNLOG
                # sort bedfile
                echo "bedSort ${j}.rmdup.bed ${j}.rmdup.s.bed"
                bedSort ${j}.rmdup.bed ${j}.rmdup.s.bed 2>> $RUNLOG
                ...
                bedtools subtract -A -a ${j}.rmdup.s.bed -b ${BLACK_LIST_FILE} > ${j}.${bedname}.blacklist.bed 2>> $RUNLOG
                echo "bedtools subtract -A -a ${j}.${bedname}.blacklist.bed -b ${REPEAT_MASKER_FILE} > ${j}.${bedname}.bed"
                bedtools subtract -A -a ${j}.${bedname}.blacklist.bed -b ${REPEAT_MASKER_FILE} > ${j}.${bedname}.bed 2>> $RUNLOG
                ...
                bedSort ${j}.${bedname}.black.rmsk.bed ${j}.${bedname}.bed 2>> $RUNLOG
            ...
            "yes|no")
                ...
                # sort bedfile
                echo "bedSort ${j}.rmdup.bed ${j}.rmdup.s.bed"
                bedSort ${j}.rmdup.bed ${j}.${bedname}.bed 2>> $RUNLOG
            ...
            "no|yes")
                ...
                bamToBed -i ${j}.s.bam > ${j}.bed 2>> $RUNLOG
                # sort bedfile
                echo "bedSort ${j}.rmdup.bed ${j}.rmdup.s.bed"
                bedSort ${j}.bed ${j}.s.bed 2>> $RUNLOG
                ...
                bedtools subtract -A -a ${j}.s.bed  -b ${BLACK_LIST_FILE} > ${j}.${bedname}.blacklist.bed 2>> $RUNLOG
                ...
                echo "This part will add one "read" back over the repeat sequences. It will not add it back over the blasklisted regions since this messes up downstream processing."
                echo "cat ${j}.${bedname}.bed ${REPEAT_MASKER_FILE} > ${j}.${bedname}.black.rmsk.bed"
                cat ${j}.${bedname}.bed ${REPEAT_MASKER_FILE} > ${j}.${bedname}.black.rmsk.bed 2>> $RUNLOG
                bedSort ${j}.${bedname}.black.rmsk.bed ${j}.${bedname}.bed 2>> $RUNLOG
        ...
        if  [ ${rmdup_rmsk} = "no|no" ]; then
            ...
            /rothenberg/sulu/data00/homer/bin/makeTagDirectory ${j}${suffix} ${j}.${refgenome}.sam -format sam 2>> $RUNLOG
            ...
            # convert sam to bam
            samtools view -bS -o ${j}.bam ${j}.${refgenome}.sam 2>> $RUNLOG
            # convert bam to bed
            bamToBed -i ${j}.bam > ${j}.bed 2>> $RUNLOG
            # sort bed-file
            ...
            genomeCoverageBed -bg -i ${j}.${refgenome}.s.bed -g ${REF_DIR_SIZE} -scale $scalevar > ${j}.${refgenome}.bg 2>> $RUNLOG
            #convert bedgraph to bigwig file
            echo "Generating ${j}.${refgenome}.bw"
            bedGraphToBigWig ${j}.${refgenome}.bg ${REF_DIR_SIZE} ${j}.${refgenome}.bw 2>> $RUNLOG
            ...
            /rothenberg/sulu/data00/homer/bin/makeTagDirectory ${j}${suffix} ${j}.${bedname}.bed -forceBED 2>> $RUNLOG

            # sort bed-file
            bedSort ${j}.${bedname}.bed ${j}.${refgenome}.s.bed 2>> $RUNLOG
            # get normaliztion factor for bigwig file
            scalevar=$( cat ${j}.${refgenome}.s.bed | wc -l )
            scalevar=$( echo "scale=4; 1000000/$scalevar" | bc )
            echo "**Inverse of Number of Reads (in Millions)**  $scalevar"
